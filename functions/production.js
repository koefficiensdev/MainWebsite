...
const {hash}=require('./production-service');
const {generate}=require('./bespoke-production');
...
exports.generateProductionArtifacts=onCall({...options,timeoutSeconds:900,memory:'512MiB',secrets:[aiKey]},async request=>{
  if(request.auth?.token?.admin!==true)throw new HttpsError('permission-denied','Admin access required');
  if(Buffer.byteLength(JSON.stringify(request.data||{}))>100000)throw new HttpsError('invalid-argument','Túl nagy kérés.');
  try{return await generate(getFirestore(),request.data||{},request.auth.uid,aiKey.value());}catch(error){if(error.code==='AI_MONTHLY_BUDGET_EXCEEDED')throw new HttpsError('resource-exhausted','A meglévő havi AI-költségkeret nem enged új tervezést.');throw new HttpsError(['invalid-argument','failed-precondition','already-exists','aborted','not-found','unavailable'].includes(error.code)?error.code:'internal',error.code?error.message:'A gyártás eredménye bizonytalan; ugyanazt a kérést próbáld újra.');}
});
exports.productionPreview=onRequest({timeoutSeconds:30,maxInstances:2,cors:false},async(req,res)=>{
  ...
  const html=row.files[file].replace(/href="(index|marketing|rolunk|szolgaltatasok|kapcsolat)\.html"/g,(_all,name)=>`href="?job=${job}&amp;token=${token}&amp;file=${name}.html"`);
