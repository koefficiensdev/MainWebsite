...
async function postOverpass(query) {
  ...
  return new Promise((resolve, reject) => {
    const req = https.request({ hostname: "overpass.kumi.systems", path: "/api/interpreter", method: "POST", timeout: 50000, headers: { "Content-Type": "application/x-www-form-urlencoded", "Content-Length": Buffer.byteLength(body), "User-Agent": "OVEXI-LeadResearch/1.0 (+https://ovexi.hu)" } }, response => {
      if (response.statusCode !== 200) { response.resume(); return reject(new Error("OSM_UNAVAILABLE")); }
...
module.exports = { discoverOsm, normalizeElements, osmPlan, osmQuery };
