...
const input = () => ({ requestId: "e4a1bcdb-7b27-4655-a50a-1f59aaa0ff21", contactName: "Teszt Elek", companyName: "Teszt szalon", email: "teszt@example.com", itemIds: ["website-business"], infrastructurePlan: "new", businessDescription: "Időpont alapján dolgozó fodrászat.", primaryGoal: "Időpontfoglalás", termsAccepted: true, operatingCostsAcknowledged: true, businessPurchaseConfirmed: true, hungarianBillingConfirmed: true });
...
test("website orders require a valid domain and hosting plan", () => {
  ...
  for (const infrastructurePlan of ["","not_applicable","forged"]) assert.throws(() => d.validateOrder({ ...input(), infrastructurePlan }), /domainnel/);
  assert.equal(d.validateOrder({ ...input(), itemIds:["marketing-launch"], infrastructurePlan:"forged" }).infrastructurePlan, "not_applicable");
});
...
test("every ready package produces an exact HUF Checkout line item", () => {
  ...
  for (const product of ready) {
    ...
    assert.equal(payload.line_items.length, 1, product.id);
    assert.equal(payload.line_items[0].price_data.unit_amount, product.price * 100, product.id);
    assert.equal(payload.mode, product.billing === "monthly" ? "subscription" : "payment", product.id);
    assert.deepEqual(payload.line_items[0].price_data.recurring, product.billing === "monthly" ? { interval:"month" } : undefined, product.id);
  }
});
