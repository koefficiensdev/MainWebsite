...
test("performance: storefront defers Firebase until consent or order submission",()=>{const js=read("js/main.js"),analytics=read("js/site-analytics.js"),css=read("css/site.css");assert.doesNotMatch(js,/^import .*firebase/m);assert.match(analytics,/async function sdk\(\)/);assert.match(analytics,/if\(!consent\(\)\)return/);assert.doesNotMatch(css,/fonts\.googleapis\.com/);assert.ok(fs.statSync(path.join(root,"assets/images/logo-256.png")).size<30000);assert.ok(fs.statSync(path.join(root,"assets/images/og-ovexi-1200.jpg")).size<250000);});
