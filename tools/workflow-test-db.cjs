...
module.exports=function memoryDb(){
  ...
  function collection(path,filters=[]){return {path,orderBy:()=>collection(path,filters),limit:()=>collection(path,filters),doc:id=>doc(`${path}/${id}`),where:(field,op,value)=>{if(op!=='==')throw Error('Unsupported query');return collection(path,[...filters,[field,value]]);},get:async()=>query(path,filters),filters};}
  ...
  return {records,collection,doc,async runTransaction(fn){
