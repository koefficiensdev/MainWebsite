...
import {STATUS_LABELS as labels,SOURCE_NAMES,outreachReady,escapeHtml as e,safeUrl,dateMs,dayKey,inPeriod,money,providerSent,summarize,validateCampaign,validateExpense,csv} from "./admin-model.js?v=20260905-2";
...
function render(){
  ...
  if(from&&to&&from>to){$("coverageNote").innerHTML='<p class="error-note">A kezdő dátum nem lehet későbbi a záró dátumnál.</p>';return;}
  const summary=summarize(data,from,to);
  const stateOf=key=>states[key].status,
    label=key=>`${SOURCE_NAMES[key]}: ${data[key].length}${states[key].more?"+":""}`,
    rank={error:0,loading:1,idle:2,ready:3},
    ordered=sources.slice().sort((a,b)=>(rank[stateOf(a)]??2)-(rank[stateOf(b)]??2)),
    counts=sources.reduce((acc,key)=>{acc[stateOf(key)]=(acc[stateOf(key)]||0)+1;return acc;},{}),
    trouble=(counts.error||0)+(counts.idle||0)+(counts.loading||0),
    roll=trouble?`${sources.length} adatforrás · ${counts.ready||0} betöltve · ${trouble} hiányzik`
                :`${sources.length} adatforrás · mind betöltve`;
  $("coverageNote").innerHTML=`<details class="source-summary"${trouble?"":" data-all-ready"}>`
    +`<summary><span class="source-dot${trouble?" is-warn":""}"></span>${e(roll)}`
    +`<span class="source-hint">Az összesítés a dátumszűrőt követi; a keresés csak a listákat szűri.</span></summary>`
    +`<div class="source-grid">${ordered.map(key=>`<span class="source-chip is-${e(stateOf(key))}">${e(label(key))}</span>`).join("")}</div>`
    +`<p class="small">A + jelű listák régebbi adatai külön betölthetők; createdAt nélküli régi rekordok nem szerepelnek.</p></details>`;
...
function metric([title,total,note]){const empty=total==="—";return `<article class="metric"><span>${e(title)}</span><strong${empty?' class="is-empty"':""}>${e(total)}</strong><small>${e(note)}</small></article>`;}
function renderProposalRequests(){
  const needLabels={website:"Weboldal / megújítás",inquiries:"Érdeklődők kezelése",appointments:"Időpontok",quotes:"Ajánlatkérések",administration:"Kevesebb adminisztráció",unsure:"Még bizonytalan"};
  const rows=search(periodRows("proposal_requests"));
  $("proposalRequestsList").innerHTML=rows.map(r=>`<article class="record-card"><div class="record-top"><div>${badge(r.status)}<h3>${e(r.companyName||"—")}</h3><p>${e(r.proposalNumber||r.id)} · ${formatDate(r.createdAt)}${r.source==="social_landing"?` · ${e(r.campaignSource||"közösségi céloldal")}`:""}</p></div></div><div class="record-details"><div><strong>Kapcsolat</strong>${e(r.contactName)}<br>${e(r.email)}<br>${e(r.phone||"Nincs telefon")}</div><div><strong>Vállalkozás</strong>${e(r.businessType)}<p>${link(r.currentUrl)}</p></div><div><strong>Kért területek</strong>${(Array.isArray(r.needs)?r.needs:[]).map(item=>e(needLabels[item]||item)).join("<br>")}${r.campaignName?`<p>${e(r.campaignName)} · ${e(r.campaignContent||"nincs kreatívnév")}</p>`:""}</div></div><details><summary>Leírt nehézség és kapcsolatfelvétel</summary><p class="preserve">${e(r.mainChallenge)}</p><p>Kapcsolatfelvételt kérte: ${r.contactConsent===true?"igen":"nem igazolt"}<br>Adatkezelést megismerte: ${r.privacyAccepted===true?"igen":"nem igazolt"}<br>Verzió: ${e(r.consentVersion||"—")}</p></details><div class="record-actions"><label>Állapot<select data-status-select="${e(r.id)}" data-type="proposal_requests">${options(["new","qualified","contacted","won","completed","cancelled"],r.status)}</select></label><button data-save-status="${e(r.id)}" data-type="proposal_requests" type="button">Állapot mentése</button><a class="button-link" href="mailto:${encodeURIComponent(r.email||"")}?subject=${encodeURIComponent(`OVEXI javaslat · ${r.companyName||"vállalkozás"} · ${r.proposalNumber||""}`)}">Válasz e-mailben →</a></div></article>`).join("")||sourceEmpty("proposal_requests","Még nincs beérkezett személyre szabott javaslatkérés.");
}
function renderOrders(){
  const filter=$("orderFilter").value,rows=search(periodRows("orders")).filter(r=>filter==="all"||r.status===filter);
  $("ordersList").innerHTML=rows.map(r=>`<article class="record-card"><div class="record-top"><div>${badge(r.status)}<h3>${e(r.companyName||"—")}</h3><p>${e(r.orderNumber||r.id)} · ${formatDate(r.createdAt)}</p></div><div class="record-total">${money(r.onceTotal)}<br><span class="muted">${money(r.monthlyTotal)} / hó</span></div></div><div class="record-details"><div><strong>Kapcsolat</strong>${e(r.contactName)}<br>${e(r.email)}<br>${e(r.phone)}</div><div><strong>Csomagok</strong>${(Array.isArray(r.itemNames)?r.itemNames:[]).map(e).join("<br>")}<p>Fizetés: ${e(r.paymentStatus||"Nincs visszaigazolva")}<br>Számla: ${e(r.invoiceStatus||"Nincs")}</p></div><div><strong>Brief</strong>${e(r.businessDescription)}<p>${e(r.primaryGoal)} · ${e(r.tone)}</p></div></div><details><summary>Teljes brief és további adatok</summary><p>Célközönség: ${e(r.targetAudience||"—")}</p><p>Meglévő oldal: ${link(r.currentUrl)}</p><p>Promókód: ${e(r.promoCode||"—")}</p>${r.promotion?.label?`<p>Promóció: ${e(r.promotion.label)}</p>`:""}<p>Megjegyzés: ${e(r.notes||"—")}</p><p>Egyeztetés oka: ${e(r.reviewReason||"—")}</p></details><div class="record-actions"><label>Igény besorolása<select data-status-select="${e(r.id)}" data-type="orders">${options([...new Set([r.status,"new","needs_review","qualified","cancelled"])],r.status)}</select></label><button data-save-status="${e(r.id)}" data-type="orders" type="button">Besorolás mentése</button><button type="button" data-go="customers">Munkafolyamat és előnézet →</button></div></article>`).join("")||sourceEmpty("orders","Nincs a szűrésnek megfelelő rendelés.");
}
