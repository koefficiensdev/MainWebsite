...
export function sourceName(value){const source=String(value||'direct');if(source==='direct')return'Közvetlen';if(source==='google_organic')return'Google kereső';if(source==='bing_organic')return'Bing kereső';if(source==='google_ads')return'Google Ads';if(source==='meta_ads')return'Meta hirdetés';if(source==='tiktok_ads')return'TikTok hirdetés';if(source==='social')return'Közösségi oldal';if(source==='internal')return'Belső oldal';if(source.startsWith('utm:'))return source.slice(4);if(source.startsWith('referral:'))return source.slice(9);return source;}
const paidMedium=value=>/(^|[_-])(paid|ads?|cpc|ppc)([_-]|$)/i.test(String(value||''));
const isPaid=row=>row.paidTraffic===true||paidMedium(row.campaignMedium)||['google_ads','meta_ads','tiktok_ads'].includes(row.source);
const platformLabel=row=>{const value=String(row.adPlatform||row.campaignSource||'').toLowerCase();if(value==='tiktok')return'TikTok';if(['instagram','ig','instagram_reels'].includes(value))return'Instagram';if(['meta','facebook','fb'].includes(value))return'Meta / Facebook';if(value==='google')return'Google Ads';return value||'Egyéb hirdetés';};
function paidPerformance(firstViews,rows,keyFn){
  const groups=new Map(),conversionSessions=new Set(rows.filter(row=>['proposal_submitted','order_submitted'].includes(row.eventType)).map(row=>row.sessionId));
  for(const row of firstViews.filter(isPaid)){const label=keyFn(row),current=groups.get(label)||{label,sessions:0,conversions:0};current.sessions+=1;if(conversionSessions.has(row.sessionId))current.conversions+=1;groups.set(label,current);}
  return [...groups.values()].map(row=>({...row,conversionRate:row.sessions?row.conversions/row.sessions*100:0})).sort((a,b)=>b.sessions-a.sessions||b.conversions-a.conversions);
}
export function analyzeAnalytics(input,from='',to=''){
  ...
  const firstViews=[...firstViewBySession.values()],paidSessions=firstViews.filter(isPaid),paidSessionIds=new Set(paidSessions.map(row=>row.sessionId)),paidConversions=new Set(conversions.filter(row=>paidSessionIds.has(row.sessionId)).map(row=>row.sessionId)).size;
  return {rows,metrics:{events:rows.length,sessions:sessionIds.size,views:views.length,conversions:conversions.length,conversionRate:sessionIds.size?conversions.length/sessionIds.size*100:0,averageDuration,shortRate:sessionIds.size?shortSessions/sessionIds.size*100:0,scroll50:scrollSet(50),scroll75:scrollSet(75),paidSessions:paidSessionIds.size,paidConversions,paidConversionRate:paidSessionIds.size?paidConversions/paidSessionIds.size*100:0},pages:pathViews,sources,referrers,devices,browsers,targets,trends,adPlatforms:paidPerformance(firstViews,rows,platformLabel),adCampaigns:paidPerformance(firstViews,rows,row=>row.campaignName||'Névtelen kampány'),adCreatives:paidPerformance(firstViews,rows,row=>`${platformLabel(row)} · ${row.campaignName||'Névtelen kampány'} · ${row.campaignContent||'Nincs hirdetésnév'}`),funnel:[{label:'Látogató',value:sessionIds.size},{label:'Legalább 30 mp',value:[...duration.values()].filter(v=>v>=30).length},{label:'CTA-kattintás',value:new Set(clicks.map(r=>r.sessionId).filter(Boolean)).size},{label:'Kosárba tette',value:eventSessions('add_to_cart')},{label:'Ajánlat / rendelés',value:new Set(conversions.map(r=>r.sessionId).filter(Boolean)).size}]};
}
