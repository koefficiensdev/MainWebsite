...
export function installOutreach({functions,getData,isAdmin,refresh,messageRows,formatDate,badge,sourceEmpty}) {
  ...
  function render(){
    ...
    const data=getData();
    $("researchJobs").innerHTML=(data.outreach_research||[]).slice(0,10).map(r=>`<div class="compact-row"><div><strong>${e(r.criteria)}</strong><p>${r.targetMode==="no_website"?"Weboldal nélküli":"Korábbi / megújítási keresés"} · Kért: ${e(r.count)} · Elkészült: ${e(r.found||0)} · ${formatDate(r.createdAt)}${r.estimatedCostUsd!=null?` · Becsült API-költség: ${Number(r.estimatedCostUsd).toFixed(3)} USD`:""}</p><p>${e(r.errorCode||"")}</p></div>${badge(r.status)}</div>`).join("");
